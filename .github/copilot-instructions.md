...
- **Baseline result (Glasgow / Southern Scotland, whole-system, 10 kWh, derived 7.42p/kWh deg cost, £13,336 capex).** Data year **2023** for all three sources (tariffs, demand, PV), aligned positionally. Headline whole-system counterfactual = **grid-only on the flat tariff (26.42p)**; **run-to-fade** is the baseline replacement policy (0.60 SOH floor, warranty-residual credit); forced-10-year replacement is a conservative sensitivity. Baseline (Agile) headline annual saving **£871.48/yr** (operating £428.43); **run-to-fade NPV −£2,186.93**; forced-10yr sensitivity NPV **−£6,572.03** (BCR 0.650). Full tariff-axis run-to-fade NPVs (flat / E7 / Agile) under the new capex are being regenerated by the parameter sweep. Same-tariff grid-only is reported as a decomposition (tariff-switch effect vs PV-BESS hardware value; baseline tariff-switch +£235.36). **NPV, not payback, is the headline viability metric.**
